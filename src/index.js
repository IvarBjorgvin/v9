...
function renderResults(location, results) {
  ...
  const body = results.map((forecast) => {
    const hour = forecast.time.split('T')[1].slice(0, 5); // Format time as HH:MM
    return el(
      'tr',
      {},
      el('td', {}, hour),
...
function render(container, locations, onSearch, onSearchMyLocation) {
  ...
  intro.textContent = 'Veldu staðsetningu til að sjá veðurspána þar.';
  ...
  locationsListElement.appendChild(
    renderLocationButton('Leita út frá staðsetningu', onSearchMyLocation)
  );
